...
class TalemyPostController: BaseController {
    ...
    @IBOutlet weak var ColleciontsView: UICollectionView!{
        didSet {
            self.ColleciontsView.AddRegisterCell([TalemyPostColl.self])
        }
    }
    ...
    override func viewDidLoad() {
        ...
        self.NameTitle.text = "My post"
        self.ColleciontsView.addSubview(self.LankView)
        self.LankView.snp.makeConstraints { make in
            make.center.equalTo(self.ColleciontsView)
        }
    ...
    private func requestDefinitionCall(){
        ShowLoad()
        GetDefinitionBlock(["tailgunning":"Story Posts","frigidarium":UserID]) { armonica, success in
            HideLoad()
            if success {
                self.dataArray.removeAll()
                let ardol = TaleBeListModel(armonica)
                self.dataArray.append(contentsOf: ardol.armonica ?? [])
                self.LankView.isHidden = self.dataArray.count > 0 ? true : false
                self.ColleciontsView.reloadData()
            }
        }
    }
}
extension TalemyPostController : UICollectionViewDelegate, UICollectionViewDataSource, UICollectionViewDelegateFlowLayout,WaterfallMutiSectionDelegate {
    ...
    func collectionView(_ collectionView: UICollectionView, cellForItemAt indexPath: IndexPath) -> UICollectionViewCell {
        let cell = collectionView.AddDequCell(TalemyPostColl.self, for: indexPath)
